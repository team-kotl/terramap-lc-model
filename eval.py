...
def sliding_window_inference(
    model, image_tensor, patch_size=512, overlap=32, device="cpu"
):
    """
    Run sliding-window inference to avoid OOM.
    - image_tensor: (1, C, H, W)
    - patch_size: size of tiles to process
    - overlap: overlap in pixels to reduce seams
    """
    ...
    with torch.no_grad():
        for y in range(0, H, stride):
            for x in range(0, W, stride):
                ...
                pred_mask[y1:y2, x1:x2] = pred  # overwrite works with overlap
...
def load_and_pad(image_path, mask_path):
    ...
    mask_padded = np.pad(
        mask, ((0, pad_h), (0, pad_w)), mode="constant", constant_values=IGNORE_INDEX
    )
...
def evaluate_model(model_path, image_path, mask_path, output_dir):
    ...
    metrics = calculate_metrics(
        final_pred.flatten(), final_mask.flatten(), NUM_CLASSES, IGNORE_INDEX
    )
    ...
    with open(os.path.join(output_dir, "metrics.txt"), "w") as f:
        ...
        for i in LEGEND_INDICES:
            ...
            f.write(
                "{:<20} {:<10.4f} {:<10.4f} {:<10}\n".format(
                    CLASS_NAMES[i], metrics["iou"][i], metrics["f1_score"][i], support
                )
            )
    ...
    # Write TIFF file
    output_tif_path = os.path.join(output_dir, "2021_CAR-lcm.tif")
